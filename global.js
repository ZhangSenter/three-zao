...
exports.config = {
  ...
  auth_cookie_name: process.env.AUTH_COOKIE_NAME || 'threezao_secret',
  login_path: '/user/login',//用户登录地址
  time_zone: 8,//时区，不般不用改
  admin_user_email: process.env.ADMIN_USER_EMAIL || 'lim.lu@cloudminds.com',//默认超级管理员的邮箱地址
  nodeMailer: {
    service: "Gmail",
...
var MONGO_URL = "mongodb://localhost/three-zao";
exports.MONGO_URL = MONGO_URL;

//mongodb settings for mongolab START
//如果使用 MongoLab 提供的 MongoDB 服务，请保留这个配置，否则，删除下面这一行
exports.database = mongoskin.db(process.env.MONGOLAB_URI || MONGO_URL);//数据库连接串
