...
exports.createObjectIDFromHexString = function(originalHex) {
    return ObjectID.createFromHexString(originalHex);
}
...
exports.getToday = function (cb) {
    var start_t = dateformat(new Date(), 'yyyy-mm-dd ') + "00:00:00";
    var end_t = dateformat(new Date(), 'yyyy-mm-dd ') + "23:59:59";

    console.log('---##---' + start_t);
    console.log('---##---' + end_t);

    //根据时间范围进行查找
    db.order.find({'create_time': {$gt: start_t, $lt: end_t}}).toArray(function (err, orders) {
        ...
        if (!err) {
            ...
            var forEach = function (i) {
                ...
                if (!group[shop_id]) {
                    db.shop.findOne({'_id': ObjectID.createFromHexString(shop_id)}, function (err, shop) {
                        group[shop_id] = {
                            shop: shop,
                            totalPrice: 0,
                            totalNum: 0,
